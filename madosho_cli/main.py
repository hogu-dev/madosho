...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("create-kb", help="create a server-owned KB in a corpus")
    p.add_argument("corpus")
    p.add_argument("name")
    _add_json(p)
    p.set_defaults(func=commands.cmd_create_kb)

    p = sub.add_parser("list-kbs", help="list server-owned knowledge bases")
    _add_json(p)
    p.set_defaults(func=commands.cmd_list_kbs)

    p = sub.add_parser("get-kb-page", help="print one KB page (whole page)")
    p.add_argument("kb_id", type=int)
    p.add_argument("slug")
    _add_json(p)
    p.set_defaults(func=commands.cmd_get_kb_page)
    ...
    _add_json(p)
    p.set_defaults(func=commands.cmd_add_kb_page)

    p = sub.add_parser("edit-kb-page", help="edit a KB page's body/description")
    p.add_argument("kb_id", type=int)
    p.add_argument("slug")
    ...
    p.add_argument("--body-file", default=None, dest="body_file")
    _add_json(p)
    p.set_defaults(func=commands.cmd_edit_kb_page)

    p = sub.add_parser("search-kb", help="full-text search over a KB's pages")
    p.add_argument("kb_id", type=int)
    p.add_argument("query")
    _add_json(p)
    p.set_defaults(func=commands.cmd_search_kb)
